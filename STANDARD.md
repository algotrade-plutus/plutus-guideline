...
>
> *This document is the standard itself, written for human readers. New to the repo? The
> [README](README.md) orients you first. For a step-by-step walkthrough of making one
> project compliant, read the [GUIDE](GUIDE.md). The exhaustive technical contract
> (schemas, exit codes, tolerances) lives in the [REFERENCE](REFERENCE.md).*

---

## Why the Plutus Reproducibility Standard?
...
Reproducibility Standard. Everything else in this document — the file formats, the scoring, the
badges — exists to make reproducibility checkable rather than something you take on
faith.

The standard makes that claim *mechanical*. Reproducible means one command,
...
> ALGOTRADE's shared datasets are documented in [archive/v1/data/DATA.md](archive/v1/data/DATA.md).
...
The Plutus score is an LLM-assessed reference signal, not a certified quality grade, and
...
The *Tidy* bucket (25 points) scores your documentation. A fill-in-the-blanks
template that walks each section is at
[templates/README.template.md](templates/README.template.md); copy it to start. A strong
Plutus README covers, at minimum:
...
- *Pin your Python.* Include a `.python-version` (or equivalent) so the container
  rebuilds against the same interpreter every time.
...
1. `plutus init` — scaffold the manifest, an example script, and a `.dockerignore`.
...
A growing set of projects carry Plutus badges. The canonical reference is
...
This is Version 2 of the Plutus Reproducibility Standard, targeting `plutus-verify` ≥ 0.5.1
